...
> 一个**前后端分离**的自习室座位预约管理系统：学生在线选座预约、签到签退；管理员可自定义楼层/区域并可视化绘制楼层结构图，统一管理座位、公告与用户。
>
...
- ✅ 签到 / 签退：预约状态自动流转（待签到 → 使用中 → 已完成），超时未签到记为违约（**扣 100 信用积分**，扣至 0 触发 24 小时禁约）
- 📋 预约记录：查看自己的历史预约与当前状态，可取消未开始的预约
- 📢 公告互动：查看公告、发表评论、删除自己的评论、点赞

### 🛠️ 管理员端

- 👥 用户管理：新增学生/管理员、启用/禁用、重置密码、删除、**调整信用积分**（扣至 0 自动禁约 24 小时）
- 📝 公告管理：新增、编辑、发布 / 下架、删除公告
- 🏢 楼层 / 区域管理：自定义名称并创建楼层与区域；楼层删除需先清空其下区域 / 座位 / 结构图，删除区域会自动解绑其下座位
- 🎨 楼层结构图：自研 SVG 可视化编辑器（区域 / 墙体 / 自由画笔 / 座位 / 文字 / 底图），支持**草稿**与**发布**，发布时自动同步 `seat` 表
...
- 💎 信用积分：新用户默认 500 分，违约一次扣 100 分；扣至 0 自动禁约 24 小时，管理员可手动调整（恢复为正立即解除）
...
├── study-room-front/               # Vue 3 + TypeScript 前端
│   ├── src/api/                    # 接口封装（user/seat/announcement/floorLayout/floor）
│   ├── src/types/                  # 共享类型（api.ts / layout.ts）
│   ├── src/utils/                  # 工具与楼层绘图纯逻辑（含 Vitest 单测）
│   ├── src/components/             # 业务组件（floor-editor/ 为楼层绘图、属性面板与楼层/区域管理）
│   ├── src/views/                  # 页面
...
> 💡 如果你的数据库是**旧版本升级**，请按需执行以下升级脚本（均幂等）：
> ```sql
> SOURCE study-room-back/src/main/resources/sql/upgrade_area.sql;   -- 旧库无 area 表时执行：seat 增加 area_id 并初始化区域
> SOURCE study-room-back/src/main/resources/sql/upgrade_points.sql; -- 旧库无积分字段时执行：sys_user 增加 points / book_ban_until
> ```
...
- 管理员在「楼层绘图」中为每个楼层绘制布局：区域 / 墙体 / 自由画笔 / 通用图形（矩形、圆、三角等，可作桌椅装饰）/ 座位 / 文字 / 底图；支持左侧工具条、框选多选、复制剪切粘贴与右键平移，区域名称与座位编号可自定义字号与区域内对齐；座位可与数据库「区域」绑定（用于楼层 → 区域筛选）
- 绘图页内可打开「楼层 / 区域管理」：自定义名称并创建楼层与区域
- 绘制结果可保存**草稿**或**发布**；**发布时自动同步座位表**
- 普通用户查看座位地图时，若该楼层已发布结构图，可直接在图上点击座位预约

### 💎 信用积分与违约停约

- 新用户默认 **500 分**；违约（超时未签到）一次扣 **100 分**
- 积分扣至 0 时自动禁约 **24 小时**（个人中心会展示禁约截止时间）
- 管理员可在「用户管理」中调整积分（正数加分 / 负数扣分）；积分恢复为正时立即解除禁约
...
- 或修改 `application.yml` 中 `server.port`，同时修改前端 `vite.config.ts` 的代理目标
