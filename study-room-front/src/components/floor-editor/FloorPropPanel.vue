  ...
  <div class="fe-panel">
    ...
    <div class="fe-panel-body">
      ...
      <template v-else-if="seatObj">
        ...
        <div class="fe-field">
          <label>尺寸</label>
          <input type="number" min="24" max="100" :value="seatSize" @change="onSize($event)" />
        </div>
        <div class="fe-field">
          <label>位置 X</label>
      ...
      <div class="fe-panel-tip">
        <p>拖动对象可移动位置</p>
        <p>Delete 删除选中 · Ctrl+Z / Ctrl+Y 撤销重做</p>
        <p>按住空格拖动可平移画布</p>
      </div>
    </div>
  </div>
...
const props = defineProps<{
  kind: EditorKind
  obj: LayoutAreaShape | LayoutSeatShape | LayoutWallShape | LayoutPathShape | LayoutTextShape
  areas: AreaInfo[]
}>()
...
function onNum(key: string, e: Event): void {
