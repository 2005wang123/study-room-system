    ...
    <main class="main-content" :class="{ 'main-content-shifted': isSidebarOpen }">
      ...
      <header class="top-header">
        <button class="menu-toggle" @click="toggleSidebar">
          <svg viewBox="0 0 24 24" width="24" height="24" fill="none" stroke="currentColor">
            <path v-if="!isSidebarOpen" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" 
      ...
      <div class="seat-container">
        ...
        <div v-if="publishedLayout && selectedAreaId == null" class="layout-mode-bar">
          <span class="layout-mode-title">{{ layoutMode ? '🖼️ 楼层结构图（点击座位可预约）' : '💡 该楼层已有结构图' }}</span>
          <button class="state-btn small" @click="layoutMode = !layoutMode">{{ layoutMode ? '切换到列表视图' : '查看结构图' }}</button>
        </div>
...
const bookingSeat = ref(null);
const bookingLoading = ref(false);
const publishedLayout = ref(null);
...
const announcementReadIds = ref(loadReadIds());
...
const menuItems = computed(() => {
  const items = [
    ...
    { key: 'announcements', icon: '📢', label: '公告', badge: announcementCount.value > 0 ? String(announcementCount.value) : '' },
...
const partialCount = computed(() => seats.value.filter(s => { const r = seatRatio(s); return r > 0 && r < 1; }).length);
...
// 解析座位数据
const parseSeatData = (response) => {
  try {
    // 1. 直接就是数组
    if (Array.isArray(response)) return response;
    // 2. request.js 已解包：{ code, message, data }
    if (response?.code === 200) return extractList(response.data);
    // 3. 兼容未解包的 axios 原始响应
    if (response?.data?.code === 200) return extractList(response.data.data);
  } catch (err) {
...
const fetchSeats = async () => {
  ...
  try {
    ...
    const listData = parseSeatData(res);
...
const fetchFloors = async () => {
  try {
    ...
    error.value = err.message || '楼层加载失败';
    await fetchSeats();
  }
};
...
const fetchPublishedLayout = async (floorId) => {
  if (floorId == null) {
  ...
  try {
    ...
    let data = res;
    if (data?.code === 200) data = data.data;
    publishedLayout.value = data?.layoutJson ? JSON.parse(data.layoutJson) : null;
...
const handleSeatClick = async (seat) => {
  ...
  try {
    ...
    if (myRes) {
      ...
      if (!confirm(`取消 ${seat.seatNo || seat.id} 号座位 ${range} 的预约？`)) return;
      await cancelBooking(seat.id, myRes.id);
      await fetchSeats();
    ...
    showToast(`✗ 操作失败: ${err.message || '请重试'}`, 'error');
    await fetchSeats();
  }
};
...
const handleBookingConfirm = async (payload) => {
  ...
  try {
    ...
    const msg = err.response?.data?.message || err.message || '请重试';
...
// 子页面 toast 事件处理
const onChildToast = ({ message, type = 'info' }) => {
  showToast(message, type);
};

// 公告页加载完成后同步列表，并在公告页内自动标记已读
const onAnnouncementLoaded = (data) => {
  announcementList.value = Array.isArray(data) ? data : [];
...
const handleSwitchAccount = () => {
  if (!confirm('确认切换账户？当前账号将退出登录')) return;
  // 清除本地存储
  localStorage.removeItem('token');
  localStorage.removeItem('userInfo');
...
.stat-dot.gray { background: #9e9e9e; }
